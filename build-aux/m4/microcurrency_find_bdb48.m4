...
AC_DEFUN([MICROCURRENCY_FIND_BDB48],[
  ...
  if test "$use_bdb" = "no"; then
    ...
    if test "$bdbpath" = "X"; then
      ...
      MICROCURRENCY_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdbpath}],db_cxx)
      ...
      MICROCURRENCY_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdb48path}],db_cxx)
      bdbpath="${bdb48path}"
      use_bdb=yes
    fi
  else
