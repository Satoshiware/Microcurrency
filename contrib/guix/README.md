# Bootstrappable Microcurrency Core Builds

This directory contains the files necessary to perform bootstrappable Microcurrency
Core builds.
...
To build Microcurrency Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Microcurrency Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/microcurrency-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/microcurrency-core/guix.sigs).
