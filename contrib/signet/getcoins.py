...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing microcurrency-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='microcurrency-cli', help='microcurrency-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Microcurrency address to which the faucet should send')
...
parser.add_argument('microcurrency_cli_args', nargs='*', help='Arguments to pass on to microcurrency-cli (default: -signet)')

args = parser.parse_args()

if args.microcurrency_cli_args == []:
    args.microcurrency_cli_args = ['-signet']


def microcurrency_cli(rpc_command_and_params):
    argv = [args.cmd] + args.microcurrency_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = microcurrency_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = microcurrency_cli(['getnewaddress', 'faucet', 'bech32'])
