...
This guide describes how to build microcurrencyd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Microcurrency Core from source.
...
### 4. Clone Microcurrency repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Microcurrency Core repository to a directory.
...
It is not necessary to build wallet functionality to run `microcurrencyd` or  `microcurrency-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/satoshiware/microcurrency/blob/master/doc/descriptors.md), `sqlite` is required.
...
Microcurrency Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/satoshiware/microcurrency/issues/7714).
...
You can deploy a `.dmg` containing the Microcurrency Core application using `make deploy`.
...
## Building Microcurrency Core

### 1. Configuration

There are many ways to configure Microcurrency Core, here are a few common examples:
...
Run the following in your terminal to compile Microcurrency Core:
...
## Running Microcurrency Core

Microcurrency Core should now be available at `./src/microcurrencyd`.
If you compiled support for the GUI, it should be available at `./src/qt/microcurrency-qt`.

The first time you run `microcurrencyd` or `microcurrency-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Microcurrency/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Microcurrency"

touch "/Users/${USER}/Library/Application Support/satoshiware/microcurrency.conf"

chmod 600 "/Users/${USER}/Library/Application Support/satoshiware/microcurrency.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Microcurrency/debug.log
```

## Other commands:

```shell
./src/microcurrencyd -daemon      # Starts the microcurrency daemon.
./src/microcurrency-cli --help    # Outputs a list of command-line options.
./src/microcurrency-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/microcurrency-qt -server # Starts the microcurrency-qt server mode, allows microcurrency-cli control
```
