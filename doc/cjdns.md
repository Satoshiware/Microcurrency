# CJDNS support in Microcurrency Core

It is possible to run Microcurrency Core over CJDNS, an encrypted IPv6 network that
...
done outside of Microcurrency Core, similarly to a VPN (either in the host/OS or on
the network router).
...
redundancy and robustness for both the Microcurrency network and individual nodes.
...
## Run Microcurrency Core with CJDNS

Once you are connected to the CJDNS network, the following Microcurrency Core
...
When enabled, this option tells Microcurrency Core that it is running in an
...
IPv6 local network. This helps Microcurrency Core perform better address management:
...
CJDNS support was added to Microcurrency Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `microcurrency-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/satoshiware/microcurrency/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Microcurrency Core

There are several ways to see your CJDNS address in Microcurrency Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `microcurrency-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `microcurrency-cli getpeerinfo`).
