# PSBT Howto for Microcurrency Core

Since Microcurrency Core 0.17, an RPC interface exists for Partially Signed Microcurrency
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/microcurrency/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Microcurrency transactions that are not fully signed
...
#### Multisig with multiple Microcurrency Core instances
...
Microcurrency Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `microcurrency-cli` is used.
...
Later, when *V* MCU has been received on *Amulti*, and Bob and Carol want to
