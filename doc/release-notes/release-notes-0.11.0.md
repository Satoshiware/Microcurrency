Microcurrency Core version 0.11.0 is now available from:

  <https://microcurrency.org/bin/microcurrency-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer MCU/kB fee to be
...
For example, add the following to `microcurrency.conf`:
...
related to the blockchain in the microcurrency system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Microcurrency Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Microcurrency Core will 
...
For an example script see [microcurrency-submittx](https://github.com/laanwj/microcurrency-submittx).
...
- #5334 `cf87536` libmicrocurrencyconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new microcurrency-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only MicrocurrencyAmountField
- #5753 `0f44672` Add microcurrency logo to about screen
...
- #5809 `46bfbe7` Add microcurrency-cli man page
...
- #5849 `41259ca` contrib/init/microcurrencyd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Microcurrency-Qt.app to Microcurrency-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for microcurrency-tx)
