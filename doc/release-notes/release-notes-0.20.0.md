...
Microcurrency Core version 0.20.0 is now available from:

  <https://microcurrencycore.org/bin/microcurrency-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `MICROCURRENCY_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Microcurrency Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Microcurrency Core version 0.18.0.
...
- Testing or debugging of implementations of the Microcurrency P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Microcurrency Core. Microcurrency Core logs debug messages
...
- OpenSSL is no longer used by Microcurrency Core.  (#17265)

- BIP70 support has been fully removed from Microcurrency Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/satoshiware/microcurrency/issues/16599),
  [PR #16702](https://github.com/satoshiware/microcurrency/pull/16702), and the
  `microcurrencyd help` for more information.  This option is experimental and
...
- All custom settings configured when Microcurrency Core starts are now
...
- The "Start Microcurrency Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://microcurrencycore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Microcurrency Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Microcurrency Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/microcurrency-core/HWI) for
  signing.  Future versions of Microcurrency Core should support a GUI option
...
- Microcurrency Core's automatically-generated source code documentation is
  now available at https://doxygen.microcurrencycore.org.  (#17596)
...
- The `microcurrency-cli` utility used with the `-getinfo` parameter now
...
- The `microcurrency-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `microcurrencystrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_microcurrency-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBMICROCURRENCY_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_microcurrency` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBMICROCURRENCY_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for microcurrency-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate microcurrency schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest microcurrency.conf (sdaftuar)
...
- #18022 Fix appveyor `test_microcurrency` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (microcurrency-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone microcurrency-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Microcurrency Core files (hebasto)
...
- #17119 Fix broken microcurrency-cli examples (andrewtoth)
...
- #17503 Remove bitness from microcurrency-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.microcurrencycore.org (laanwj)
...
- #17648 Rename wallet-tool references to microcurrency-wallet (hel-o)
...
- #18341 Replace remaining literal MCU with `CURRENCY_UNIT` (domob1812)
...
- #16115 On microcurrencyd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from microcurrency-cli -getinfo (malevolent)
