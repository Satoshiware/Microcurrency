# User-space, Statically Defined Tracing (USDT) for Microcurrency Core

Microcurrency Core includes statically defined tracepoints to allow for more
...
**Note**: Microcurrency Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Microcurrency Core
...
Multiple tools can list the available tracepoints in a `microcurrencyd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Microcurrency Core, use `info probes` in `gdb`:

```
$ gdb ./src/microcurrencyd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/microcurrencyd
stap net        outbound_message 0x0000000000107c05 /src/microcurrencyd
stap validation block_connected  0x00000000002fb10c /src/microcurrencyd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Microcurrency Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/microcurrencyd | grep NT_STAPSDT -A 4 -B 2
