...
static void SetupMicrocurrencyTxArgs(ArgsManager &argsman)
{
...
{
    SetupMicrocurrencyTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " microcurrency-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  microcurrency-tx [options] <hex-tx> [commands]  Update hex-encoded microcurrency transaction\n"
                "or:     microcurrency-tx [options] -create [commands]   Create hex-encoded microcurrency transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded microcurrency transaction
