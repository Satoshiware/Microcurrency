This directory contains the source code for the Microcurrency Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### microcurrencygui.(h/cpp)

- Represents the main window of the Microcurrency UI.
...
- ClientModel is used by the main application `microcurrencygui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a microcurrency:... link from a browser).
...
* UI elements like MicrocurrencyAmountField, which inherit from QWidget.
* `microcurrencystrings.cpp`: automatically generated
* `microcurrencyunits.(h/cpp)`: MCU / mMCU / etc. handling
* `callback.h`
...
**Note:** Do not change `local/microcurrency_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `microcurrency-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `microcurrency-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `microcurrency-qt`
9. You're all set! Start developing, building, and debugging the Microcurrency Core GUI
