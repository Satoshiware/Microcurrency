...
#include <qt/microcurrencyaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Microcurrency address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new MicrocurrencyAddressEntryValidator(parent));
    widget->setCheckValidator(new MicrocurrencyAddressCheckValidator(parent));
}
...
bool parseMicrocurrencyURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no microcurrency: URI
    if(!uri.isValid() || uri.scheme() != QString("microcurrency"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MicrocurrencyUnits::parse(MicrocurrencyUnits::MCU, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMicrocurrencyURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMicrocurrencyURI(uriInstance, out);
}

QString formatMicrocurrencyURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("microcurrency:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MicrocurrencyUnits::format(MicrocurrencyUnits::MCU, info.amount, false, MicrocurrencyUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMicrocurrencyConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", MICROCURRENCY_CONF_FILENAME));
    ...
    /* Open microcurrency.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Microcurrency.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Microcurrency (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Microcurrency (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Microcurrency*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "microcurrency.desktop";
    return GetAutostartDir() / strprintf("microcurrency-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a microcurrency.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Microcurrency\n";
        else
            optionFile << strprintf("Name=Microcurrency (%s)\n", chain);
