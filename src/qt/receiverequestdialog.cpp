...
#include <config/microcurrency-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatMicrocurrencyURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(MicrocurrencyUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatMicrocurrencyURI(info));
}
