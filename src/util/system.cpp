...
const char * const MICROCURRENCY_CONF_FILENAME = "microcurrency.conf";
const char * const MICROCURRENCY_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //microcurrency-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", MICROCURRENCY_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "microcurrency";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Microcurrency
    // macOS: ~/Library/Application Support/Microcurrency
    // Unix-like: ~/.microcurrency
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Microcurrency";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Microcurrency";
#else
    // Unix-like
    return pathRet / ".microcurrency";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MICROCURRENCY_CONF_FILENAME);
