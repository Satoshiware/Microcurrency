This directory contains integration tests that test microcurrencyd and its
...
microcurrencyd and microcurrency-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (microcurrency-util, microcurrency-tx, ...).
...
Before tests can be run locally, Microcurrency Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the microcurrencyd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another microcurrencyd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its microcurrencyd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other microcurrencyd processes are running.

On linux, the test framework will warn if there is another
microcurrencyd process running when the tests are started.

If there are zombie microcurrencyd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
microcurrencyd processes running on the system, so should not be used if any non-test
microcurrencyd processes are being run.**

```bash
killall microcurrencyd
```

or

```bash
pkill -9 microcurrencyd
```
...
sure microcurrencyd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and microcurrencyd `debug.log`s will all be dumped
...
`test_framework.log` and microcurrencyd `debug.log`s can be combined into a single
...
call methods that interact with the microcurrencyd nodes-under-test.

If further introspection of the microcurrencyd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/microcurrencyd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/microcurrencyd <pid>
```
