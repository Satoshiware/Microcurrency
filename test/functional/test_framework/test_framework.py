...
TMPDIR_PREFIX = "microcurrency_func_test_"
...
class MicrocurrencyTestMetaClass(type):
    """Metaclass for MicrocurrencyTestFramework.

    Ensures that any attempt to register a subclass of `MicrocurrencyTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'MicrocurrencyTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("MicrocurrencyTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("MicrocurrencyTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class MicrocurrencyTestFramework(metaclass=MicrocurrencyTestMetaClass):
    """Base class for a microcurrency test script.

    Individual microcurrency test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave microcurrencyds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop microcurrencyds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use microcurrency-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_microcurrencyd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "microcurrencyd" + config["environment"]["EXEEXT"],
        )
        fname_microcurrencycli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "microcurrency-cli" + config["environment"]["EXEEXT"],
        )
        self.options.microcurrencyd = os.getenv("MICROCURRENCYD", default=fname_microcurrencyd)
        self.options.microcurrencycli = os.getenv("MICROCURRENCYCLI", default=fname_microcurrencycli)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: microcurrencyds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("MicrocurrencyRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'microcurrencyd', self.options.microcurrencyd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'microcurrency-cli', self.options.microcurrencycli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                microcurrencyd=binary[i],
                microcurrency_cli=binary_cli[i],
                version=versions[i],
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.microcurrencyconf
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple microcurrencyds"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a microcurrencyd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple microcurrencyd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as microcurrencyd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    microcurrencyd=self.options.microcurrencyd,
                    microcurrency_cli=self.options.microcurrencycli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in microcurrency.conf
    ...
    def skip_if_no_microcurrencyd_zmq(self):
        """Skip the running test if microcurrencyd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("microcurrencyd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if microcurrency-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("microcurrency-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if microcurrency-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("microcurrency-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether microcurrency-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether microcurrency-wallet was compiled."""
